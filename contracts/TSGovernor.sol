    ...
    GovernorVotesQuorumFraction
{
    ...
    mapping(uint256 => uint256) private voteIds;
    ...
    function propose(
        ...
        uint256 voteId = uint256(keccak256(abi.encode(keccak256(bytes(description)))));
        require(voteIds[voteId]==0,"TSGovernor: voteId exist");
        voteIds[voteId] = voteId;
        ...
        if(!voteRefund) {
            ...
            require(tsVesting.rate()+percent<=tsVesting.BPS(),"TSGovernor: percent invalid");
    ...
    function castVote(uint256 proposalId, uint8 support) public virtual override returns (uint256) {
        require(TSVesting(tokenVesting).getBalanceVesting(_msgSender())>0,"TS: weight zero");
